...
function chooseDate(e) { 
  console.log("function chooseDate is called!haniiiiiiiiiiiiiii!")
...
function onDOMReady(fn,ctx){
  ...
  {if(e&&e.type=="DOMContentLoaded"){fireDOMReady()}
  else if(e&&e.type=="load"){fireDOMReady()}
  else if(document.readyState)
  {if((/loaded|complete/).test(document.readyState)){fireDOMReady()}
...
function onSelect() {
  console.log("onSelect is executed");
  dateObject.date1=null;
  dateObject.date2=null;
  dateObject.operator=selecter.value;
  console.log("db yallah khtarina range li brina:  ",dateObject);
    const innerHTML1 = '<input type="text" class="datepicker id="date1">';
    const innerHTML2 = '<input type="text" class="datepicker id="date2">';
  ...
  switch(selecter.value){
        ...
        default://BETWEEN
           ...
           //add the date picker + the image and on click the image execute function show datePicker twice for each datepicker
